	...
	COMSCORE.SiteRecruit.Utils = ( function() {
		...
		return {
			...
			getDevice: function() {
				...
				var b = {};
				if (ua.test(navigator.userAgent)) {
					// Phone
					b.type = 2;
					if (mv >= 770) {
						// Tablet
				...
				try {
					...
					if (str.length > 4) {
						if (window.JSON && window.JSON.parse) {
							...
							obj.sv = _pArr[0].substring(_pArr[0].indexOf(":") + 1);
							obj.state = {};
 ...
 COMSCORE.SiteRecruit.Utils.UserPersistence = {
	...
	getUserObj: function(params) {
		...
		if (uservalue && uservalue!=""){
			...
			//For future dev, we probably want to increment this sv if we want
			//to prevent scripts of different sv from accessing certain cookies
			if (userObj && userObj.sv)
			{
			    return userObj;
			}
			else 
			...
			*/
			...
			run: function() {
				...
				if (this.config.mobile) {
					...
					if (this.config.mobile.halt == true && mEx.test(navigator.userAgent)) {
						return;
					}
				}
